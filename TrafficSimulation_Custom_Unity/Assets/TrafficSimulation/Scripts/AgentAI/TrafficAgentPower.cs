...
namespace TrafficSimulation
{
    ...
    public class TrafficAgentPower : MonoBehaviour
    {
        ...
        private void ApplyInput()
        {
            ...
            float combinedInput = Mathf.Clamp01(Mathf.Min(_speedLimitInput, _stopPointInput)) -
                                  Mathf.Clamp01(Mathf.Max(-_speedLimitInput, -_stopPointInput));
        ...
        private void UpdateSpeedLimitInput()
        {
            ...
            float speedAlongDirection =
                Vector3.Dot(_agent.CarBehaviour.Velocity, _agent.CurrentSample.DirectionForward);
            ...
            _speedLimitInput = Mathf.MoveTowards(_speedLimitInput, targetSpeedLimitInput,
                _agent.Settings.SpeedLimitInputMaxChangeRate * Time.deltaTime);
        }


        private void UpdateStopPointInput()
        {
            // Stop point based on sensor
            float relativeVelocity = Vector3.Dot(_agent.FrontSensorResult.Velocity, _agent.CarBehaviour.Forward) *
                                     _agent.Settings.VelocityOfHitColliderStrength;
            float sensorStoppingDistanceOffset =
                _agent.AgentSize.z * _agent.Settings.StoppingDistanceVehicleLengthMultiplier +
                _agent.Settings.DefaultStoppingDistance;
            float sensorStopDistance =
                _agent.FrontSensorResult.Distance - sensorStoppingDistanceOffset + relativeVelocity;

            // Stop point based on segment
            float segmentStopDistance = _agent.CurrentSample.DistanceToSegmentEnd - _agent.AgentSize.z * 0.5f;
            
            if(_agent.NextSegment != null && _agent.IntersectionState == TrafficAgent.IntersectionStateType.None)
                segmentStopDistance += _agent.NextSegment.TotalLength;
            
            // Ignore the segment stop distance if the agent is moving
            if (_agent.IntersectionState == TrafficAgent.IntersectionStateType.Moving)
                segmentStopDistance = float.MaxValue;

            // Pick the smallest stop distance
